...
describe("browser extension", () => {
	...
	it("sends the independent token and turns an artifact into an image block", async () => {
		...
		await withEnvironment(
			{ MEWA_BROWSER_URL: "http://browser.test:8787/", MEWA_BROWSER_TOKEN: "browser-secret" },
			async () => {
		...
		expect(calls[1]?.init?.headers).toEqual({ authorization: "Bearer browser-secret" });
	});
	...
	it("fails closed when a service response points outside its artifact route", async () => {
		await withEnvironment(
			{ MEWA_BROWSER_URL: "http://browser.test:8787", MEWA_BROWSER_TOKEN: "secret" },
			async () => {
				await expect(
					executeBrowserRequest(
						{ command: "screenshot", session: "qa", args: ["screen.png"] },
