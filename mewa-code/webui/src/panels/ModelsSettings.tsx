			...
			<div className="flex items-start justify-between gap-sm">
				<div className="flex flex-col gap-xs">
					...
					<p className="text-text-muted tr-text-metadata">
						{availableCount} available · {visibleCount} shown. Hiding changes Mewa catalog views
						only; Pi keeps the canonical catalog.
					</p>
				</div>
...
export function ModelRow({
	...
	const cache = cacheText(model.cost);
	return (
		<div
		...
		>
				...
				<div className="mt-xs flex flex-wrap items-center gap-xs text-text-muted tr-text-metadata">
					...
					{model.reasoning ? (
						...
						>
							...
							Reasoning
							{model.thinkingLevels.length > 0 ? ` · ${model.thinkingLevels.join("/")}` : ""}
						</span>
					) : null}
				</div>
			</div>
			...
			<div className="flex min-w-[9rem] flex-col items-end gap-0.5 text-right">
				<span className="rounded-[var(--radius-xs)] bg-control-bg-selected px-sm py-xs text-text-default tr-text-metadata">
					{formatTokenCount(model.contextWindow)} ctx · {formatTokenCount(model.maxTokens)} out
				</span>
