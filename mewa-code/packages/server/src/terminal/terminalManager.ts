...
import { type IPty, type IPtyForkOptions, spawn } from "bun-pty";
import {
	loadConfig,
...
let ptySpawner: TerminalPtySpawner = spawn;

export function setTerminalPtySpawner(spawner: TerminalPtySpawner): void {
	ptySpawner = spawner;
}

export function terminalSshLaunch(
	cwd: string,
	config: SshConfig = loadSshConfig(),
): { file: "ssh"; args: string[]; options: IPtyForkOptions } {
	return {
		file: "ssh",
		args: buildSshArgs(config, {
			allocatePty: true,
			remoteCommand: remoteTerminalCommand(cwd),
		}),
		options: {
			name: "xterm-256color",
			env: { ...sshClientEnvironment(), TERM: "xterm-256color", COLORTERM: "truecolor" },
		},
	};
}
...
function spawnForTab(
	...
	const launch = terminalSshLaunch(ws.worktreePath);
	const pty = ptySpawner(launch.file, launch.args, { ...launch.options, ...grid });
