...
import { isProtectedPath, protectedStateRoots } from "./agent/protected-paths";
...
function configuredRootEntries(env: NodeJS.ProcessEnv): string[] {
	const raw = env.MEWA_MOUNT_ROOTS?.trim();
	if (!raw) {
		throw new Error(
			"MEWA_MOUNT_ROOTS is required. Configure one or more absolute same-path bind-mount roots.",
		);
	}
	const separator = raw.includes(",") ? "," : delimiter;
	const entries = raw
		.split(separator)
		.map((entry) => entry.trim())
		.filter(Boolean);
	if (entries.length === 0) {
		throw new Error("MEWA_MOUNT_ROOTS must contain at least one absolute directory");
	}
	return entries;
}
...
export function configuredMountRoots(env: NodeJS.ProcessEnv = process.env): string[] {
	...
	for (const display of configuredRootEntries(env)) {
		if (!isAbsolute(display)) {
			throw new Error(`MEWA_MOUNT_ROOTS entries must be absolute: ${display}`);
		}
		const lexical = resolve(display);
		if (lexical === "/") throw new Error("MEWA_MOUNT_ROOTS must not contain /");
		let canonical: string;
		try {
			const stats = lstatSync(lexical);
			if (!stats.isDirectory()) {
				throw new Error(`MEWA_MOUNT_ROOTS entry is not a directory: ${display}`);
			}
			canonical = canonicalExisting(lexical);
		} catch (error) {
			if (error instanceof Error && error.message.startsWith("MEWA_MOUNT_ROOTS entry")) {
				throw error;
			}
			throw new Error(`MEWA_MOUNT_ROOTS entry is missing or not mounted: ${display}`, {
				cause: error,
			});
		}
		if (canonical === "/") throw new Error("MEWA_MOUNT_ROOTS must not contain /");
		if (protectedRoots.some((root) => isWithin(root, canonical) || isWithin(canonical, root))) {
			throw new Error(`MEWA_MOUNT_ROOTS entry overlaps protected controller state: ${display}`);
		}
		if (!roots.some((root) => root.path === canonical)) roots.push({ display, path: canonical });
	}
...
function outsidePathError(path: string, label: string): Error {
	return new Error(`${label} is outside the approved same-path mounts (MEWA_MOUNT_ROOTS): ${path}`);
}
...
export function resolveMountedPath(candidate: string, options: MountPathOptions = {}): string {
	...
	if (isProtectedPath(absolute, { env, roots: protectedCanonicalRoots(env) })) {
		throw new Error(`${label} is protected Pi or Mewa state: ${absolute}`);
	}
