...
function resolveInWorktree(
	...
	const root = assertMountedDirectory(ws.worktreePath, "Workspace");
	...
	if (options.allowMissingLeaf) {
		...
		assertMountedPath(parent, { label: "File parent" });
		return { root, abs, ...(mode === undefined ? {} : { mode }) };
	}

	const real = assertMountedPath(abs, { label: "Workspace file" });
