...
function nameTaken(project: Project, candidate: string): boolean {
	return branchExists(project.path, candidate) || existsSync(worktreePathFor(project, candidate));
}

function worktreePathFor(project: Project, branch: string): string {
	return join(
		dirname(project.path),
		`.${basename(project.path)}-mewa-worktrees`,
		project.slug,
		branch,
	);
}
...
export function listExistingWorktrees(projectId: string): ExistingWorktreeCandidate[] {
	...
	return entries.flatMap((entry): ExistingWorktreeCandidate[] => {
		let path: string;
		try {
			path = assertMountedDirectory(entry.path, "Existing worktree");
		} catch {
			return [];
		}
		if (entry.prunable || path === projectPath || representedPaths.has(path)) return [];
		return entry.branch
			? [{ path, branch: entry.branch, status: "available" }]
...
export function openExistingWorktree(projectId: string, requestedPath: string): Workspace {
	...
	const mountedRequestedPath = assertMountedDirectory(requestedPath, "Existing worktree");
	const wantedPath = canonicalPath(mountedRequestedPath);
	const projectPath = canonicalPath(project.path);
	...
	const workspace: Workspace = {
		...
		name: displayName,
		branch: entry.branch,
		worktreePath: assertMountedDirectory(entry.path, "Existing worktree"),
		baseBranch,
		renamed: true,
	};
	all.push(workspace);
...
function folderTruth(repoPath: string): { branch: string; baseBranch: string } {
	const mounted = assertMountedDirectory(repoPath, "Workspace");
	return { branch: currentBranch(mounted), baseBranch: resolveDefaultBranch(mounted) };
}
...
export async function createWorkspace(
	...
	const worktreePath = worktreePathFor(project, branch);
	assertMountedPath(worktreePath, { allowMissingLeaf: true, label: "Managed worktree" });
...
export function ensureWorkspaceScratchDir(ws: Workspace): void {
	const worktreePath = assertMountedDirectory(ws.worktreePath, "Workspace");
	if (!statSync(worktreePath, { throwIfNoEntry: false })?.isDirectory())
		throw new Error(`Workspace directory is missing: ${worktreePath}`);
...
export function refreshUserOwnedWorkspace(workspaceId: string): void {
	...
	assertMountedDirectory(peek.worktreePath, "Workspace");
	const truth =
		peek.kind === "default"
...
export function listWorkspaces(
	...
	for (const workspace of rows) assertMountedDirectory(workspace.worktreePath, "Workspace");
...
export function reclaimWorktree(ws: Workspace): void {
	...
	if (!project) return;
	const projectPath = assertMountedDirectory(project.path, "Project");
	assertMountedDirectory(ws.worktreePath, "Managed worktree");
	if (resolve(ws.worktreePath) === resolve(projectPath)) return;
	const removed = git(projectPath, ["worktree", "remove", "--force", ws.worktreePath]);
	if (!removed.ok) {
...
export function getWorkspace(id: string): Workspace {
	const ws = loadWorkspaces().find((w) => w.id === id);
	if (!ws) throw new Error(`Unknown workspace: ${id}`);
	assertMountedDirectory(ws.worktreePath, "Workspace");
	return ws;
}
