...
describe("authentication token validation", () => {
	...
	test("accepts a strong printable token and canonical protocol encoding", () => {
		expect(isStrongToken(strongToken)).toBe(true);
		const protocol = encodeCodeTokenProtocol(strongToken);
		expect(protocol).toBeDefined();
		expect(decodeCodeTokenProtocol(protocol)).toBe(strongToken);
	});
});
