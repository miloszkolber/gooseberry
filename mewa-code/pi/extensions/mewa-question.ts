...
const questionSchema = Type.Object({
  prompt: Type.String({ description: "The focused question to ask the user." }),
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (options.length > 0) {
        ...
        const selected = await ctx.ui.select(params.prompt, choices, { signal });
        answer = selected === customLabel
          ? await ctx.ui.input(params.prompt, "Enter your answer", { signal })
          : selected;
      } else {
        answer = await ctx.ui.input(params.prompt, "Enter your answer", { signal });
      }

      if (answer === undefined) throw new Error("The user cancelled the question.");
