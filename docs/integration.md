...
Goose remains authoritative for sessions, history, providers, models, credentials, extensions, tools, compaction, permissions, recipes, and scheduler state. Gooseberry exposes projects, goals and agent-owned tasks, supporting questions, custom-agent summon, Git and files, native session fork with immediate-parent lineage, session rename and reversible archive projections, history and slash-command projections, and focused provider, model, extension, tool, recipe, and scheduler controls. Objective updates and supporting questions use the authenticated, session-scoped MCP endpoint. Browser automation uses a lazy Goose skill and the separate `gooseberry-browser` HTTP service. [`acp.md`](acp.md) records the current protocol projection.
