...
Concurrent persistent chats support streaming replies and thinking, tool results, images, permissions, questions, usage/context, steering, interruption, queued follow-ups, model/thinking choices, forks, rename, archive, search, slash commands and agent mentions.

Each chat has a user-set goal and ordered agent-managed tasks: `pending`, `active` or `done`. Session-scoped MCP lets agents update tasks and ask questions. Browser MCP provides automation and screenshots.

Goose stores transcripts. Follow-up queues survive browser reconnects but not controller restarts.
