...
A project contains one or more allowed directory roots and can include zero, one or several Git repositories. Users choose and name projects in the UI. Each root must be mounted read-only in the application container at its host path.
...
Each chat can have one user-set goal and an ordered list of agent-managed tasks: `pending`, `active` or `done`. Session-scoped MCP lets agents update tasks and ask supporting questions. Agent mentions and editing use the user's own Goose agents. Browser tools and guidance come from the separate browser MCP service.
...
The user installs and manages official upstream Goose on the host. Two host-networked containers run the application and browser service as separate executables from one Go module. Application and browser state live under `${GOOSEBERRY_DATA_PATH}/app` and `${GOOSEBERRY_DATA_PATH}/browser`, mounted only into their respective services.
...
This is a single-user application. Goose has the Linux user's host permissions. Chromium can access browser state and the host network, but has no application-state, project or Goose-configuration mounts. See [security](security.md), [deployment](deployment.md) and [ACP coverage](acp.md).
