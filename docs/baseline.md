...
Each session may have one goal and an ordered task list with `pending`, `active`, and `done` states. Users set the goal, while agents own task creation and state through objective MCP. The same session-scoped MCP service lets an agent pause and ask the user supporting questions. Custom agents (`scout`, `builder`, `strategist`, and `auditor`) run through Goose. The browser capability is a lazy skill backed by the separate Go browser HTTP service. Settings project Goose provider status, API-key configuration, native OAuth/device-code authentication, ACP readiness, models, two allowlisted preferences, persisted global provider/model defaults, and a focused agent catalog. Canonical metadata and per-million-token prices are shown when Goose reports them. The catalog edits bounded plain-text agent instructions, name, description, and the upstream model-ID preference for global agents and agents in an admitted project directory. Goose retains source paths, arbitrary source properties, configuration, credentials, and all persisted state. Settings also project recipes, schedules, global extensions, active-session extensions, active-session tool inventory, and Goose's global tool permissions. Extension controls expose only sanitized identity and descriptive metadata, while Goose retains raw configuration and credentials. Schedule controls include run, pause, resume, inspection, termination, recent sessions, update, and deletion.
