...
func measureTreeUsage(path, root string, ignoreTemporary bool) (int64, int, error) {
	...
	for _, entry := range entries {
		...
		count++
		if info.IsDir() {
			size, entries, err := measureTreeUsage(child, root, ignoreTemporary)
			if err != nil {
				return 0, 0, err
			}
			total += size
...
func (a *app) runBrowser(ctx context.Context, request browserRequest) (map[string]any, error) {
	...
	stateBytes, stateEntries, err := measureTreeUsage(stateDir, stateDir, false)
	if err != nil {
		return fail(err)
	}
	a.accounting.Lock()
	...
	for !waited {
		...
		case <-statePoll.C:
			if !closing {
				...
				currentStateBytes, currentStateEntries, monitorErr = measureTreeUsage(stateDir, stateDir, false)
	...
	finalStateBytes, finalStateEntries, err := measureTreeUsage(stateDir, stateDir, false)
	if err != nil {
		return fail(err)
	}
