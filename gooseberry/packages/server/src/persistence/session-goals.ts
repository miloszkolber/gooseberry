...
/** Atomically updates either or both objective fields after validating the complete replacement. */
export function updateStoredSessionObjective(
	projectId: string,
	sessionId: string,
	...
	const current = readStoredObjective(projectId, sessionId);
	const goal = "goal" in update ? normalizeSessionGoal(update.goal) : (current?.goal ?? null);
	const tasks = "tasks" in update ? normalizeTasks(update.tasks) : (current?.tasks ?? []);
	return writeObjective(projectId, sessionId, goal, tasks);
}
