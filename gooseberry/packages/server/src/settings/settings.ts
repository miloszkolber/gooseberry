...
type SettingsPolicyReconciler = (config: AppConfig) => void;

let publishSettings: SettingsPublisher | null = null;
let reconcileSettingsPolicy: SettingsPolicyReconciler | null = null;

export function setSettingsPublisher(fn: SettingsPublisher | null): void {
	publishSettings = fn;
}

export function setSettingsPolicyReconciler(fn: SettingsPolicyReconciler | null): void {
	reconcileSettingsPolicy = fn;
}
...
export function updateConfig(partial: AppConfigPatch): AppConfig {
	...
	const next: AppConfig = {
		signet: normalizeSignetSettings({
			...current.signet,
			...(partial.signet ?? {}),
		}),
		hiddenModels: normalizeModelReferences(partial.hiddenModels ?? current.hiddenModels),
	};
	saveConfig(next);
	cached = next;
	reconcileSettingsPolicy?.(next);
...
export async function getSignetStatus(): Promise<import("@gooseberry/contracts").SignetStatus> {
