	...
	switch (event.type) {
		...
		case "text": {
			...
			const message = {
				...
				content: appendAssistantText(
					existing?.kind === "assistant" ? existing.message.content : [],
					event.text ?? "",
				),
			};
			const turn: ChatTurn = { kind: "assistant", id, message, streaming: true };
			return {
				...rt,
				isStreaming: true,
		...
		case "thinking": {
			const id = rt.currentAssistantId ?? crypto.randomUUID();
			const existing = rt.turns.find((turn) => turn.id === id && turn.kind === "assistant");
			const content = existing?.kind === "assistant" ? [...existing.message.content] : [];
			const prior = content.at(-1);
			if (prior?.type === "thinking") prior.thinking = `${prior.thinking}${event.text ?? ""}`;
		...
		case "tool-start": {
			if (!event.toolCallId) return rt;
			const id = rt.currentAssistantId ?? crypto.randomUUID();
			const existing = rt.turns.find((turn) => turn.id === id && turn.kind === "assistant");
			const content = existing?.kind === "assistant" ? [...existing.message.content] : [];
			if (!content.some((part) => part.type === "toolCall" && part.id === event.toolCallId))
				content.push({
					type: "toolCall",
					id: event.toolCallId,
			...
			return {
				...
				toolResults: {
					...rt.toolResults,
					[event.toolCallId]: { status: "running", raw: event.tool ?? { title: event.toolName } },
				},
			};
		}
		case "tool-update":
