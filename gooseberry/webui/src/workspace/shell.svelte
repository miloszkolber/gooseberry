...
function signOut(): void {
	void logoutController().finally(() => window.dispatchEvent(new Event("gooseberry-auth-lost")));
}
</script>
	...
	{:else if availability === "incompatible"}
		<main class="app-content flex h-full min-h-0 min-w-0 items-center justify-center px-xl py-xl text-center">
			<div class="app-status-copy max-w-[34rem]">
				<h1 class="app-status-title">{$appStore.agentProfile?.name || "Connected agent"} is not compatible with Gooseberry</h1>
				<p role="alert" class="app-status-description">The agent must support the ACP session operations Gooseberry uses to list and reopen chats.</p>
	...
	{:else if availability === "disconnected" || availability === "error"}
		<main class="app-content flex h-full min-h-0 min-w-0 items-center justify-center px-xl py-xl text-center">
			<div class="app-status-copy max-w-[30rem]">
				...
				<p role="alert" class="app-status-description">{availability === "disconnected" ? "Gooseberry will reconnect automatically. Your open work remains in this browser." : "The controller is connected, but the agent status could not be read."}</p>
