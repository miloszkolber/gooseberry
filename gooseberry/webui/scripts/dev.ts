...
export function parseDevPort(value = process.env.GOOSEBERRY_UI_DEV_PORT): number {
	if (value === undefined || value === "") return defaultPort;
	if (!/^[0-9]+$/.test(value)) throw new Error("GOOSEBERRY_UI_DEV_PORT must be an integer");
	const port = Number(value);
	if (!Number.isSafeInteger(port) || port < 1024 || port > 65_535) {
		throw new Error("GOOSEBERRY_UI_DEV_PORT must be between 1024 and 65535");
	}
	return port;
}
...
(() => {
  ...
  const poll = async () => {
    try {
      const next = await fetch("/.gooseberry-dev-version", { cache: "no-store" }).then((response) => response.ok ? response.text() : "");
...
async function publishBuild(stagedOutput: string, servedOutput: string): Promise<void> {
	...
	await rename(versionTemporary, join(servedOutput, ".gooseberry-dev-version"));
}
...
async function main(): Promise<void> {
	...
	const devRoot = await mkdtemp(join(tmpdir(), "gooseberry-web-dev-"));
	const servedOutput = join(devRoot, "web");
	const fixtureBinary = join(devRoot, "gooseberry-ui-fixture");
	...
	try {
		...
		child = Bun.spawn([fixtureBinary], {
			...
			env: {
				...process.env,
				GOOSEBERRY_ALLOWED_ORIGINS: "",
				GOOSEBERRY_ALLOW_UNAUTHENTICATED_REMOTE: "false",
				GOOSEBERRY_AUTH_ENABLED: "false",
				GOOSEBERRY_BROWSER_AUTH: "false",
				GOOSEBERRY_BROWSER_PUBLIC_ORIGIN: "",
				GOOSEBERRY_BROWSER_TOKEN: "",
				GOOSEBERRY_CONTROLLER_HOST: "127.0.0.1",
				GOOSEBERRY_PUBLIC_ORIGIN: origin,
				GOOSEBERRY_TOKEN: "",
				GOOSEBERRY_UI_FIXTURE_BASE: fixtureBase,
				GOOSEBERRY_UI_FIXTURE_PORT: String(port),
				GOOSEBERRY_UI_FIXTURE_READY_FILE: fixtureReady,
				GOOSEBERRY_UI_STATIC_DIR: servedOutput,
			},
			stdout: "inherit",
			stderr: "inherit",
		});
		...
		console.log(`dev: Gooseberry UI is available at ${origin}`);
