...
func TestACPInitialToolPayload(t *testing.T) {
...
func TestACPToolInputPatch(t *testing.T) {
	m, _, p, _ := newSessionManager(t, []map[string]any{
...
func TestACPDistinctMessageIDs(t *testing.T) {
	m, _, p, _ := newSessionManager(t, []map[string]any{
...
func TestACPMessageUsageIsReplacedByIdentity(t *testing.T) {
	updates := []map[string]any{}
	for _, tokens := range []int{100, 100, 120} {
		updates = append(updates, map[string]any{"__gooseOnly": true, "sessionUpdate": "message_usage", "messageId": "usage-one", "usage": map[string]any{"inputTokens": tokens, "outputTokens": 20, "totalTokens": tokens + 20, "cost": 0.25}})
	}
...
func TestACPConfigSelectorUsesAdvertisedIdentityAndValues(t *testing.T) {
