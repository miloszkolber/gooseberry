...
func TestRealBrowserPanelLeaseLifecycle(t *testing.T) {
	executable := os.Getenv("GOOSEBERRY_TEST_AGENT_BROWSER")
	if executable == "" {
	...
	config, err := browser.ConfigFromEnvironment(func(key string) (string, bool) {
		values := map[string]string{"GOOSEBERRY_BROWSER_AUTH": "true", "GOOSEBERRY_BROWSER_TOKEN": testToken}
		value, ok := values[key]
		return value, ok
	})
	if err != nil {
	...
	config.BrowserConfig = os.Getenv("GOOSEBERRY_TEST_BROWSER_CONFIG")
