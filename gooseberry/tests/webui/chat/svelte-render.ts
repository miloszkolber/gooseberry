...
function componentModule(path: string): Promise<unknown> {
	...
	pending = (async () => {
		const entrypoint = path.startsWith("tests/")
			...
			: join(webuiRoot, path);
		const virtualEntry = "gooseberry-svelte-ssr-entry";
			...
			plugins: [
				{
					...
					setup(build) {
						build.onResolve({ filter: /^gooseberry-svelte-ssr-entry$/ }, () => ({
							path: virtualEntry,
							namespace: "gooseberry-svelte-test",
						}));
						build.onLoad({ filter: /.*/, namespace: "gooseberry-svelte-test" }, () => ({
