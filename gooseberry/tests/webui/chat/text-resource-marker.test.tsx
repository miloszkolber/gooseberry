...
test("keeps text resource attachment markers through replay without resource contents", () => {
	...
	const markup = renderToStaticMarkup(
		<ChatTurnView row={{ kind: "user", id: "resource-only", message: finalTurn.message }} />,
	);
...
test("reconnect replay replaces an optimistic text attachment without retaining its source", () => {
	...
	const turns = useAppStore.getState().sessions[sessionId]?.turns ?? [];
