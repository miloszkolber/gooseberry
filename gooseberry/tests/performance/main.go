...
func run() error {
	...
	for name, endpoint := range map[string]*target{"CANDIDATE": &candidate, "REFERENCE": &reference} {
		if err := endpoint.login(os.Getenv("GOOSEBERRY_BENCH_"+name+"_TOKEN"), *unauthenticated); err != nil {
...
func (t *target) login(token string, unauthenticated bool) error {
	...
	if token == "" {
		return fmt.Errorf("set GOOSEBERRY_BENCH_CANDIDATE_TOKEN and GOOSEBERRY_BENCH_REFERENCE_TOKEN in the private environment")
	}
