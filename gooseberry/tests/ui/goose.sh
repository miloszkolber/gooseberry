#!/bin/sh
set -eu
session=gooseberry-goose-acceptance
fixture_pid=
browser() { agent-browser --session "$session" "$@"; }
assert_eval() { browser eval "(() => { if (!($1)) throw new Error('Goose UI acceptance assertion failed: ' + JSON.stringify('$2')); return true; })()" >/dev/null; }
cleanup() {
 status=$?
 trap - EXIT INT TERM
 if [ "$status" -ne 0 ]; then
  browser screenshot /artifacts/goose-failure.png >/dev/null 2>&1 || true
  browser get text body > /artifacts/goose-failure.txt 2>&1 || true
  cat /artifacts/goose-failure.txt >&2 || true
 fi
...
GOOSEBERRY_UI_FIXTURE_GOOSE=1 /app/gooseberry-ui-fixture > /artifacts/goose-fixture.log 2>&1 &
fixture_pid=$!
attempt=0
while [ ! -s /tmp/gooseberry-ui-ready ]; do
 attempt=$((attempt + 1))
 if ! kill -0 "$fixture_pid" 2>/dev/null || [ "$attempt" -ge 100 ]; then echo 'Goose fixture did not start' >&2; exit 1; fi
 sleep 0.1
done
...
for theme in light dark; do
 ...
 for width in 320 390 768 1024 1440; do
  ...
  for tab in Providers Models Goose Automation Tools Signet System; do
   ...
   browser screenshot "/artifacts/goose-${theme}-${width}-${tab}.png" >/dev/null
  done
  echo "Goose settings checked: $theme ${width}px"
 done
done
...
browser screenshot /artifacts/goose-short-settings.png >/dev/null
browser set viewport 390 844 >/dev/null
browser eval 'new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))' >/dev/null
browser find role tab click --name Goose >/dev/null
...
browser screenshot /artifacts/goose-login-recovery.png >/dev/null
...
browser screenshot /artifacts/goose-prompt-recovery.png >/dev/null
browser eval "window.reviewFailures.clear();window.reviewFailures.add('session.permissionReply');window.reviewSocket.dispatchEvent(new MessageEvent('message',{data:JSON.stringify({channel:'session.permissionRequest',data:{sessionId:'fixture-1',id:'review-permission',toolCallId:'review-tool',title:'Run synthetic command',tool:{title:'Run synthetic command',rawInput:{command:'echo synthetic'}},options:[{optionId:'allow',name:'Allow once',kind:'allow_once'},{optionId:'reject',name:'Reject',kind:'reject_once'}]}})}));true" >/dev/null
browser wait --fn 'document.querySelector("[data-testid=tool-permission-dialog]")?.open === true' >/dev/null
browser find text 'Tool request details' click >/dev/null
assert_eval "document.querySelector('[data-testid=tool-permission-dialog] pre')?.textContent?.includes('echo synthetic')" 'permission-only tool context is inspectable'
browser find role button click --name 'Allow once' >/dev/null
browser wait --fn 'document.querySelector("[data-testid=tool-permission-dialog] [role=alert]") !== null' >/dev/null
assert_eval "document.querySelector('[data-testid=tool-permission-dialog]')?.open" 'rejected permission reply remains actionable'
browser screenshot /artifacts/goose-permission-recovery.png >/dev/null
echo 'Goose UI acceptance passed'
