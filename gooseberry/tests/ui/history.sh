...
session=gooseberry-history-measurement
fixture_pid=
...
export GOOSEBERRY_UI_HISTORY_ROUNDS="${GOOSEBERRY_UI_HISTORY_ROUNDS:-1000}"
/app/gooseberry-ui-fixture > /artifacts/history-fixture.log 2>&1 &
fixture_pid=$!
attempt=0
...
attempt=0
while [ "$(browser eval 'globalThis.__gooseberryHistory?.done === true')" != true ]; do
	attempt=$((attempt + 1))
...
browser eval '(() => { const state = globalThis.__gooseberryHistory; if (state.error) throw new Error(state.error); return state.result; })()' > /artifacts/history.json
