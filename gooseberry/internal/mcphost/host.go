...
func (m *browserModule) Descriptor() ModuleDescriptor {
	return ModuleDescriptor{
		ID: "browser", ExtensionName: "gooseberry-browser", DisplayName: "Gooseberry Browser",
...
func NewService(config Config, build diagnostics.BuildInfo, logger *slog.Logger) (*Service, error) {
	...
	if config.Authentication && !strongToken(config.Token) {
		return nil, fmt.Errorf("GOOSEBERRY_MCP_TOKEN must be a strong printable random token")
	}
	if config.PublicOrigin != "" {
		normalized, err := normalizeOrigin(config.PublicOrigin)
		if err != nil || !config.Authentication {
			return nil, fmt.Errorf("GOOSEBERRY_MCP_PUBLIC_ORIGIN requires authentication and one absolute http(s) origin without a path")
		}
	...
	for _, id := range config.Modules {
		if _, ok := moduleFactories[id]; !ok {
			return nil, fmt.Errorf("unknown Gooseberry MCP module %q", id)
		}
		if active[id] {
			return nil, fmt.Errorf("Gooseberry MCP module %q is configured more than once", id)
		}
		active[id] = true
	}
	...
	for _, id := range config.DisabledModules {
		...
		if disabled[id] {
			return nil, fmt.Errorf("Gooseberry MCP disabled module %q is configured more than once", id)
		}
		disabled[id] = true
		delete(active, id)
	}
