...
func ConfigFromEnvironment(lookup func(string) (string, bool)) (Config, error) {
	...
	host := strings.TrimSpace(environmentValue(lookup, "GOOSEBERRY_MCP_HOST", defaultHost))
	port, err := positiveInteger(environmentValue(lookup, "GOOSEBERRY_MCP_PORT", strconv.Itoa(defaultPort)), "GOOSEBERRY_MCP_PORT")
	if err != nil {
		return Config{}, err
	}
	authentication, err := exactBoolean(environmentValue(lookup, "GOOSEBERRY_MCP_AUTH", "true"), "GOOSEBERRY_MCP_AUTH")
	if err != nil {
		return Config{}, err
	}
	token := strings.TrimSpace(environmentValue(lookup, "GOOSEBERRY_MCP_TOKEN", ""))
	if authentication && !strongToken(token) {
		return Config{}, fmt.Errorf("GOOSEBERRY_MCP_TOKEN must be a strong printable random token")
	}
	publicOrigin := strings.TrimSpace(environmentValue(lookup, "GOOSEBERRY_MCP_PUBLIC_ORIGIN", ""))
	if publicOrigin != "" {
		publicOrigin, err = normalizeOrigin(publicOrigin)
		if err != nil || !authentication {
			return Config{}, fmt.Errorf("GOOSEBERRY_MCP_PUBLIC_ORIGIN requires authentication and one absolute http(s) origin without a path")
		}
	}
	if err := validateHost(host, authentication); err != nil {
		return Config{}, err
	}
	modules, err := parseModuleList(environmentValue(lookup, "GOOSEBERRY_MCP_MODULES", "browser"), "GOOSEBERRY_MCP_MODULES")
	if err != nil {
		return Config{}, err
	}
	disabled, err := parseModuleList(environmentValue(lookup, "GOOSEBERRY_MCP_DISABLED_MODULES", ""), "GOOSEBERRY_MCP_DISABLED_MODULES")
	if err != nil {
		return Config{}, err
	}
	for _, id := range append(append([]string{}, modules...), disabled...) {
		if _, ok := moduleFactories[id]; !ok {
			return Config{}, fmt.Errorf("unknown Gooseberry MCP module %q", id)
		}
	}
...
func browserLookup(lookup func(string) (string, bool), host string, port int, authentication bool, token, publicOrigin string) func(string) (string, bool) {
	return func(key string) (string, bool) {
		if strings.HasPrefix(key, "GOOSEBERRY_BROWSER_") {
			suffix := strings.TrimPrefix(key, "GOOSEBERRY_BROWSER_")
			if value, ok := lookup("GOOSEBERRY_MCP_" + suffix); ok {
				return value, true
			}
			switch suffix {
			case "HOST":
...
func validateHost(host string, authentication bool) error {
	...
	if parsed == nil {
		return fmt.Errorf("GOOSEBERRY_MCP_HOST must be localhost or an IP address")
	}
	if !parsed.IsLoopback() && !authentication {
		return fmt.Errorf("non-loopback MCP binding requires GOOSEBERRY_MCP_AUTH=true")
	}
	return nil
}
