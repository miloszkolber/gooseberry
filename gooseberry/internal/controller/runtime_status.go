...
func newRuntimeStatusProvider(build diagnostics.BuildInfo, requests *diagnostics.RequestCounter, projects *workspace.Projects, settings *Settings, staticDir string, agent *GooseClient, auth AuthConfig) *runtimeStatusProvider {
...
func (s *runtimeStatusProvider) snapshot(ctx context.Context) runtimeStatusReport {
	...
	go func() { agentResult <- projectAgentStatus(runtimeGooseStatus(bounded, s.agent)) }()
