...
type gooseAgentSource struct {
	path, name, description, content string
	global, writable                 bool
	properties                       map[string]any
}

func (a *GooseAdmin) handleAgents(ctx context.Context, method string, request map[string]any) (any, error) {
	projectDir := ""
	...
	params := map[string]any{"type": "agent"}
	var source gooseAgentSource
	if method != "goose.agentCreate" {
	...
	if method == "goose.agentDelete" {
		if err := a.call(ctx, "_goose/unstable/sources/delete", params, nil); err != nil {
			return nil, fmt.Errorf("couldn't remove Goose agent")
		}
		return ack(nil)
	}
	...
	response, err := a.objectCall(ctx, "_goose/unstable/sources/"+action, params)
	if err != nil {
		return nil, fmt.Errorf("couldn't save Goose agent")
	}
	...
	if entry == nil {
		return nil, fmt.Errorf("Goose returned an invalid agent")
	}
	return entry, nil
}

func (a *GooseAdmin) agentSources(ctx context.Context, projectDir string) ([]gooseAgentSource, error) {
	params := map[string]any{"type": "agent", "includeProjectSources": false}
	if projectDir != "" {
		params["projectDir"] = projectDir
	}
	response, err := a.objectCall(ctx, "_goose/unstable/sources/list", params)
	if err != nil {
		return nil, fmt.Errorf("couldn't load Goose agents")
	}
	result := make([]gooseAgentSource, 0)
...
func parseAgentSource(value any) (gooseAgentSource, error) {
	raw := mapValue(value)
	...
	if raw["type"] != "agent" || !globalOK || !writableOK || !nameOK || name == "" || !descriptionOK || !contentOK || !pathOK || path == "" || !propertiesOK {
		return gooseAgentSource{}, fmt.Errorf("Goose returned an invalid agent source")
	}
	return gooseAgentSource{path: path, name: name, description: description, content: content, global: global, writable: writable, properties: properties}, nil
}

func (s gooseAgentSource) id() string {
	scope := "project"
	if s.global {
		scope = "global"
	}
...
func (s gooseAgentSource) project() map[string]any {
