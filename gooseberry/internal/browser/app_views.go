...
<title>Gooseberry App Sandbox</title>
...
func (a *app) appViewPublicOrigin(request *http.Request) (string, error) {
	...
	if !a.expectedMCPHost(request) {
		return "", serviceFailure("invalid_app_view_origin", "app view registration used an unexpected Host", "configure GOOSEBERRY_BROWSER_PUBLIC_ORIGIN for a reverse proxy", http.StatusBadRequest, nil)
	}
	scheme := "http"
	if request.TLS != nil {
		scheme = "https"
	...
	if !ok {
		return "", serviceFailure("invalid_app_view_origin", "app view registration used an invalid Host", "configure GOOSEBERRY_BROWSER_PUBLIC_ORIGIN", http.StatusBadRequest, nil)
	}
	return origin, nil
}
