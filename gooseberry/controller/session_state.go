...
func NewSessionRecords(store Store) *SessionRecords { return &SessionRecords{store: store} }
...
func (s *SessionRecords) load() ([]ProjectSessionRecord, error) {
	...
	ok, err := readStore(s.store, "project-sessions.json", &value, validateRecords)
	if err != nil || !ok {
...
func (s *SessionRecords) save(records []ProjectSessionRecord) error {
	return writeStore(s.store, "project-sessions.json", storedSessions{Version: 2, Engine: "goose", Records: records}, validateRecords)
}
...
func NewObjectives(store Store) *Objectives { return &Objectives{store: store, now: time.Now} }
...
func (o *Objectives) ClearProject(projectID string) error {
	...
	for _, entry := range entries {
		...
		content, _, readErr := readStoredFile(filepath.Join(directory, entry.Name()))
		if readErr != nil {
			continue
		}
		var identity struct {
...
func (o *Objectives) read(projectID, sessionID string) (*storedObjective, error) {
	...
	ok, err := readStore(o.store, objectiveName(projectID, sessionID), &stored, func(value storedObjective) error { return validateObjective(value, projectID, sessionID) })
	if err != nil {
		return nil, err
	}
	if ok {
	...
	content, _, err := readStoredFile(legacyPath)
	if err != nil {
		return nil, nil
	}
	var legacy struct {
...
func (o *Objectives) write(stored storedObjective) error {
	return writeStore(o.store, objectiveName(stored.ProjectID, stored.SessionID), stored, func(value storedObjective) error { return validateObjective(value, stored.ProjectID, stored.SessionID) })
}
