...
func NewSettings(store Store, publish func(AppConfig)) *Settings {
...
func (s *Settings) getLocked() (AppConfig, error) {
	...
	ok, err := readStore(s.store, "config.json", &stored, nil)
	if err != nil {
...
func (s *Settings) Update(patch AppConfigPatch) (AppConfig, error) {
	...
	if err := writeStore(s.store, "config.json", next, nil); err != nil {
		s.mu.Unlock()
		return AppConfig{}, err
	}
	s.cached = &next
