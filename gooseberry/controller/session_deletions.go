...
func NewSessionDeletions(store Store) *SessionDeletions {
...
func (deletions *SessionDeletions) load() ([]sessionDeletion, error) {
	name := filepath.Join(deletions.store.Dir, "session-deletions.json")
	...
	if decodeStored(raw, &value, validateStoredSessionDeletions) != nil {
		return nil, fmt.Errorf("session deletion journal is unreadable")
	}
...
func (deletions *SessionDeletions) save(records []sessionDeletion) error {
	...
	return writeStore(deletions.store, "session-deletions.json", storedSessionDeletions{Version: 1, Engine: "goose", Records: records}, validateStoredSessionDeletions)
}
...
func stableDeletionAgentIdentity(identity string) bool {
	return identity == "goose" || strings.HasPrefix(identity, "agent:")
}
