...
func (h ObjectiveHandler) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	switch rpc.Method {
	case "initialize":
		writeRPCResult(response, rpc.ID, map[string]any{"protocolVersion": "2025-03-26", "capabilities": map[string]any{"tools": map[string]any{}}, "serverInfo": map[string]any{"name": "gooseberry-objectives", "version": "1"}})
	case "notifications/initialized":
		response.WriteHeader(http.StatusAccepted)
	case "tools/list":
		writeRPCResult(response, rpc.ID, map[string]any{"tools": objectiveTools()})
	case "tools/call":
