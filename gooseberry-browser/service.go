...
func configFromEnvironment(lookup func(string) (string, bool)) (config, error) {
	...
	return config{
		...
		MaxArtifactBytes: defaultArtifactLimit, MaxTotalArtifactBytes: defaultTotalArtifact, MaxStateBytes: defaultStateLimit, MaxSessions: defaultSessionLimit,
	}, nil
}
...
func newApp(config config) (*app, error) {
	...
	app := &app{config: config, reservations: map[string]int64{}, active: map[uint64]context.CancelFunc{}}
...
func measureTree(path, root string, ignoreTemporary bool) (int64, error) {
	...
	for _, entry := range entries {
		...
		if !within(root, child) {
			return 0, serviceFailure("unsafe_path", "path escaped quota root", "", 400, nil)
		}
		...
		if info.IsDir() {
			size, err := measureTree(child, root, ignoreTemporary)
			if err != nil {
				return 0, err
			}
			total += size
...
func (a *app) initializeStorage() error {
	...
	return cleanupStaleTemps(a.config.StateRoot, true)
}
...
func (a *app) reserveArtifactCapacity(bytes int64) (*artifactReservation, error) {
	a.accounting.Lock()
	defer a.accounting.Unlock()
	used, err := measureTree(a.config.ArtifactRoot, a.config.ArtifactRoot, true)
	if err != nil {
		return nil, err
	}
	var reserved int64
	...
	if used+reserved+bytes > a.config.MaxTotalArtifactBytes {
...
func (a *app) checkGlobalArtifactQuota(includeTemporary bool) error {
	a.accounting.Lock()
	defer a.accounting.Unlock()
	used, err := measureTree(a.config.ArtifactRoot, a.config.ArtifactRoot, !includeTemporary)
	if err != nil {
		return err
	}
	if used > a.config.MaxTotalArtifactBytes {
...
func (a *app) runBrowser(ctx context.Context, request browserRequest) (map[string]any, error) {
	...
	stateBytes, err := measureTree(stateDir, stateDir, false)
	if err != nil {
		return fail(err)
	}
	if !closing && (artifactBytes > a.config.MaxArtifactBytes || stateBytes > a.config.MaxStateBytes) {
		return fail(quotaError("browser session storage quota is exceeded"))
	}
	if !closing {
		if err := a.checkGlobalArtifactQuota(false); err != nil {
			return fail(err)
		}
	}
	...
	collector := &outputCollector{kill: func() { terminate(command) }}
	...
	defer poll.Stop()
	statePoll := time.NewTicker(500 * time.Millisecond)
	...
	for !waited {
		...
		case <-statePoll.C:
			if !closing {
				...
				currentStateBytes, monitorErr = measureTree(stateDir, stateDir, false)
				if monitorErr != nil || currentStateBytes > a.config.MaxStateBytes {
					stateExceeded = currentStateBytes > a.config.MaxStateBytes
					terminate(command)
				}
			}
		}
	...
	finalStateBytes, err := measureTree(stateDir, stateDir, false)
	if err != nil {
		return fail(err)
	}
	...
	if !closing && finalStateBytes > a.config.MaxStateBytes {
		a.closeSession(request.Session, stateDir, artifactDir)
	...
	if temporaryPath != "" {
		...
		a.accounting.Lock()
		total, measureErr := measureTree(a.config.ArtifactRoot, a.config.ArtifactRoot, false)
		if measureErr == nil && total > a.config.MaxTotalArtifactBytes {
