...
const FIXED_ORIGIN = "http://gooseberry-browser.invalid";
const TEMP_ARTIFACT_PATTERN = /^\.gooseberry-screenshot-[A-Za-z0-9_-]+\.tmp$/;
...
async function runBrowser(request, signal) {
	...
	try {
		...
		if (outputName) {
			...
			temporaryArtifact = join(artifactDir, `.gooseberry-screenshot-${randomUUID()}.tmp`);
				...
				void lstat(temporaryArtifact)
					...
					.catch((error) => {
						if (error?.code !== "ENOENT") {
							console.error("[gooseberry-browser] artifact quota check failed", error);
						}
					});
			}, 25);
		}
...
export async function startServer() {
	...
	console.log(`[gooseberry-browser] listening on ${host}:${port}`);
	return server;
}
